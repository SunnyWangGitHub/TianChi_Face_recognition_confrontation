...
def load_weights(mdl, name):
    ...
    model_dir = os.path.join('/notebooks/Workspace/tmp/pycharm_project_314/TianChi/Face_recognition/inception_resnet_v1', 'model')
    os.makedirs(model_dir, exist_ok=True)

    state_dict = {}
    if name == 'vggface2':
        cached_file = os.path.join(model_dir, '20180402-114759-vggface2.pt')
        if not os.path.exists(cached_file):
            print('no Downloading parameters')
        else:
            state_dict.update(torch.load(cached_file))
    elif name == 'casia-webface':
        cached_file = os.path.join(model_dir, '20180408-102900-casia-webface.pt')
        if not os.path.exists(cached_file):
            print('no Downloading parameters')
        else:
            state_dict.update(torch.load(cached_file))
    else:
        raise ValueError('Pretrained models not find!"')
